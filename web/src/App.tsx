...
export default function App() {
  ...
  return (
    <div className="layout">
      <aside className={`controls${open ? ' open' : ''}`}>
        <div className="controls-head">
          <h1 className="controls-title">시뮬레이션 설정</h1>
          <button className="controls-toggle" onClick={() => setOpen(!open)}>
            {open ? '접기' : '펼치기'}
          </button>
        </div>
      ...
      <main className="main">
        <section className="hero">
          <h2>업종 이식 시뮬레이터</h2>
          <p>
            권역&middot;업종&middot;수량을 조정하면 Shannon H' 변화, 추가 소비 효과, 추천 이식 지역을
            실시간으로 확인할 수 있습니다.
          </p>
        </section>
