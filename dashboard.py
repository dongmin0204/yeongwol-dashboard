...
def train_ai_model():
    ...
    trend_path = os.path.join(DATA_DIR, "sdc", "20260605073810_관광수요_세부지표별_추이.csv")
    if not os.path.exists(trend_path):
        return None

    df = pd.read_csv(trend_path)
    pivot = df.pivot_table(index="기준년월", columns="지표 소분류", values="지표값", aggfunc="first")

    consume_cols = ["외지인 소비액", "식음료 소비액", "숙박업 소비액", "쇼핑업 소비액", "여가서비스업 소비액"]
    feature_cols = ["숙박 비중", "타권역 방문자 비중", "1박 방문자수", "2박 방문자수", "3박 방문자수",
                    "내비게이션 음식유형 검색량", "내비게이션 숙박유형 검색량"]

    missing = [c for c in consume_cols + feature_cols if c not in pivot.columns]
    if missing:
        return None

    X = pivot[feature_cols].values
    y = pivot[consume_cols].mean(axis=1).values

    model = GradientBoostingRegressor(n_estimators=80, max_depth=3, learning_rate=0.1, random_state=42)
    model.fit(X, y)

    imp_order = np.argsort(model.feature_importances_)
    return {
        "model": model,
        "feature_cols": feature_cols,
        "feat_names": [feature_cols[i] for i in imp_order],
        "feat_importances": [model.feature_importances_[i] for i in imp_order],
        "current_X": X[-1],
...
def build_ai_prediction(cafe_n, food_n, stay_n):
    ai = train_ai_model()
    if ai is None:
        return None

    model = ai["model"]
    current_X = ai["current_X"].copy()

    current_pred = model.predict(current_X.reshape(1, -1))[0]

    after_X = current_X.copy()
    total_added = cafe_n + food_n + stay_n
    stay_boost = min(stay_n * 0.15, 8.0)
    visit_boost = min(total_added * 0.08, 5.0)
    navi_food_boost = min(food_n * 0.12, 4.0)
    navi_stay_boost = min(stay_n * 0.10, 3.0)

    cols = ai["feature_cols"]
    after_X[cols.index("숙박 비중")] += stay_boost
    after_X[cols.index("1박 방문자수")] += visit_boost
    after_X[cols.index("2박 방문자수")] += visit_boost * 0.6
    after_X[cols.index("3박 방문자수")] += visit_boost * 0.3
    after_X[cols.index("타권역 방문자 비중")] += min(total_added * 0.05, 3.0)
    after_X[cols.index("내비게이션 음식유형 검색량")] += navi_food_boost
    after_X[cols.index("내비게이션 숙박유형 검색량")] += navi_stay_boost

    after_pred = model.predict(after_X.reshape(1, -1))[0]

    return {
        "current_pred": current_pred,
        "after_pred": after_pred,
        "feat_names": ai["feat_names"],
        "feat_importances": ai["feat_importances"],
        "train_r2": ai["train_r2"],
        "top_feature": ai["top_feature"],
    }
...
def main():
    ...
    # ── AI Prediction ──
    ai_result = build_ai_prediction(cafe_n, food_n, stay_n)
    if ai_result:
        st.markdown("""
        <div class="card-title">AI 소비 전환 예측 (GradientBoosting)</div>
        <div class="card-sub">12개월 관광수요 지표로 학습한 모델이 업종 이식 후 소비지표 변화를 예측합니다.</div>
        """, unsafe_allow_html=True)
        ...
        with ai1:
            ...
            fig_imp.add_trace(go.Bar(
                y=ai_result["feat_names"], x=ai_result["feat_importances"],
                orientation="h",
                marker_color=["#3182f6" if v > 0.1 else "#e5e8eb" for v in ai_result["feat_importances"]],
            ))
            fig_imp.update_layout(**_base_layout(
                title=dict(text="소비 예측 Feature Importance", font=dict(size=14, color="#191f28")),
                height=340,
                yaxis=dict(autorange="reversed"),
                xaxis=dict(title="중요도"),
                showlegend=False,
                margin=dict(l=160, r=16, t=48, b=44),
            ))
            st.plotly_chart(fig_imp, use_container_width=True, config={"displayModeBar": False})

        with ai2:
            curr = ai_result["current_pred"]
            after = ai_result["after_pred"]
            delta = after - curr
...
<div class="kpi-row" style="margin-top:8px">
    <div class="kpi">
        <div class="kpi-label">현재 예측 소비지수</div>
        <div class="kpi-value slate">{curr:.2f}</div>
    </div>
    <div class="kpi">
        <div class="kpi-label">이식 후 예측 소비지수</div>
        <div class="kpi-value blue">{after:.2f}</div>
        <div class="kpi-sub"><span class="kpi-delta">+{delta:.2f} ({delta_pct:+.1f}%)</span></div>
    </div>
</div>
            """, unsafe_allow_html=True)
            ...
            st.markdown(f"""
<div class="recommend">
    <div class="recommend-title">AI 예측 결과</div>
    <p>GradientBoosting 모델(R2={ai_result['train_r2']:.3f}) 기준,
    업종 이식 후 소비지수가 <b>{curr:.2f}에서 {after:.2f}</b>로
    <b>{delta_pct:+.1f}%</b> 개선될 것으로 예측됩니다.
    핵심 드라이버는 <b>{ai_result['top_feature']}</b>입니다.</p>
</div>
            """, unsafe_allow_html=True)
